...
<div class="container">
	...
	<main ng-app="regApp" ng-controller="regAppCtrl">
		<div class="row" style="margin-bottom: 20px;">
		  <div class="col-xs-12 col-sm-10 col-md-8 col-lg-7">
			<form name="register" id="register" ng-submit="submitReg()" novalidate>
			...
			<div class="form-group">
			    ...
			    <select name="enrollee_grade" id="enrollee_grade" class="form-control" ng-model="reg.enrollee_grade" required>
			...
			<div class="panel-body">
			  ...
			  <select name="enrollee_no_siblings" id="enrollee_no_siblings" class="form-control" ng-model="reg.enrollee_no_siblings" required>
	...
	<script type="text/javascript">	
		...
		app.controller('regAppCtrl', function($scope, $http, $window) {						
			...
			$scope.checkIfTransferee = function() {
				
				if ($scope.reg['enrollee_stat'] == 2) {					
					$scope.isTransferee = true;					
					$scope.validate_enrollee_old_school_type = false;
					$scope.validate_enrollee_old_school_name = false;					
				} else {
					$scope.isTransferee = false;							
				}
				
			}			
			
			$scope.validate_enrollee_stat = false;
			$scope.validate_enrollee_grade = false;
			$scope.validate_enrollee_lname = false;
			$scope.validate_enrollee_fname = false;
			$scope.validate_enrollee_mname = false;			
			$scope.validate_enrollee_sex = false;
			$scope.validate_enrollee_address = false;
			$scope.validate_enrollee_dob = false;
			$scope.validate_enrollee_pob = false;
			$scope.validate_enrollee_religion = false;
			$scope.validate_enrollee_contact = false;
			$scope.validate_enrollee_email = false;			
			$scope.validate_enrollee_father_firstname = false;
			$scope.validate_enrollee_father_lastname = false;			
			$scope.validate_enrollee_father_job = false;
			$scope.validate_enrollee_father_income = false;						
			$scope.validate_enrollee_mother_lastname = false;
			$scope.validate_enrollee_mother_firstname = false;
			$scope.validate_enrollee_mother_job = false;
			$scope.validate_enrollee_mother_income = false;			
			$scope.validate_enrollee_guardian_lastname = false;
			$scope.validate_enrollee_guardian_firstname = false;
			$scope.validate_enrollee_guardian_job = false;
			$scope.validate_enrollee_guardian_income = false;
			$scope.validate_enrollee_no_siblings = false;
			$scope.validate_enrollee_indigenous = false;		
			$scope.validate_enrollee_tongue = false;
			
			$scope.submitReg = function() {
			
				if ($scope.register.enrollee_stat.$valid) {			
					if ($scope.reg['enrollee_stat'] == 2) {				
						$scope.validate_enrollee_old_school_type = $scope.register.enrollee_old_school_type.$invalid;
						$scope.validate_enrollee_old_school_name = $scope.register.enrollee_old_school_name.$invalid;						
					}				
				}
				
				if ($scope.register.enrollee_indigenous.$valid) {
					if ($scope.reg['enrollee_indigenous'] == 'yes') {			
						$scope.validate_enrollee_ethnicity = false;					
					}				
				}				
				
				$scope.validate_enrollee_stat = $scope.register.enrollee_stat.$invalid;								
				$scope.validate_enrollee_grade = $scope.register.enrollee_grade.$invalid;								
				$scope.validate_enrollee_lname = $scope.register.enrollee_lname.$invalid;								
				$scope.validate_enrollee_fname = $scope.register.enrollee_fname.$invalid;								
				$scope.validate_enrollee_mname = $scope.register.enrollee_mname.$invalid;				
				$scope.validate_enrollee_sex = $scope.register.enrollee_sex.$invalid;				
				$scope.validate_enrollee_address = $scope.register.enrollee_address.$invalid;
				$scope.validate_enrollee_dob = $scope.register.enrollee_dob.$invalid;
				$scope.validate_enrollee_pob = $scope.register.enrollee_pob.$invalid;				
				$scope.validate_enrollee_religion = $scope.register.enrollee_religion.$invalid;
				$scope.validate_enrollee_contact = $scope.register.enrollee_contact.$invalid;
				$scope.validate_enrollee_email = $scope.register.enrollee_email.$invalid;				
				$scope.validate_enrollee_father_lastname = $scope.register.enrollee_father_lastname.$invalid;
				$scope.validate_enrollee_father_firstname = $scope.register.enrollee_father_firstname.$invalid;				
				$scope.validate_enrollee_father_job = $scope.register.enrollee_father_job.$invalid;
				$scope.validate_enrollee_father_income = $scope.register.enrollee_father_income.$invalid;				
				$scope.validate_enrollee_mother_lastname = $scope.register.enrollee_mother_lastname.$invalid;
				$scope.validate_enrollee_mother_firstname = $scope.register.enrollee_mother_firstname.$invalid;
				$scope.validate_enrollee_mother_job = $scope.register.enrollee_mother_job.$invalid;
				$scope.validate_enrollee_mother_income = $scope.register.enrollee_mother_income.$invalid;				
				$scope.validate_enrollee_guardian_lastname = $scope.register.enrollee_guardian_lastname.$invalid;
				$scope.validate_enrollee_guardian_firstname = $scope.register.enrollee_guardian_firstname.$invalid;
				$scope.validate_enrollee_guardian_job = $scope.register.enrollee_guardian_job.$invalid;				
				$scope.validate_enrollee_guardian_income = $scope.register.enrollee_guardian_income.$invalid;				
				$scope.validate_enrollee_no_siblings = $scope.register.enrollee_no_siblings.$invalid;
				$scope.validate_enrollee_indigenous = $scope.register.enrollee_indigenous.$invalid;
				$scope.validate_enrollee_tongue = $scope.register.enrollee_tongue.$invalid;
				
				if ($scope.register.enrollee_indigenous.$valid) {				
					if ($scope.reg['enrollee_indigenous'] == 'yes') {					
						$scope.validate_enrollee_ethnicity = $scope.register.enrollee_ethnicity.$invalid;				
					}				
				}				

				$scope.isInvalid = !$scope.register.enrollee_stat.$invalid
				&& !$scope.register.enrollee_grade.$invalid
				&& !$scope.register.enrollee_lname.$invalid
				&& !$scope.register.enrollee_fname.$invalid
				&& !$scope.register.enrollee_mname.$invalid
				&& !$scope.register.enrollee_sex.$invalid
				&& !$scope.register.enrollee_address.$invalid
				&& !$scope.register.enrollee_dob.$invalid
				&& !$scope.register.enrollee_pob.$invalid
				&& !$scope.register.enrollee_religion.$invalid
				&& !$scope.register.enrollee_contact.$invalid
				&& !$scope.register.enrollee_email.$invalid				
				&& !$scope.register.enrollee_father_lastname.$invalid
				&& !$scope.register.enrollee_father_firstname.$invalid
				&& !$scope.register.enrollee_father_job.$invalid
				&& !$scope.register.enrollee_father_income.$invalid
				&& !$scope.register.enrollee_mother_lastname.$invalid
				&& !$scope.register.enrollee_mother_firstname.$invalid
				&& !$scope.register.enrollee_mother_job.$invalid
				&& !$scope.register.enrollee_mother_income.$invalid								
				&& !$scope.register.enrollee_guardian_lastname.$invalid
				&& !$scope.register.enrollee_guardian_firstname.$invalid
				&& !$scope.register.enrollee_guardian_job.$invalid
				&& !$scope.register.enrollee_guardian_income.$invalid
				&& !$scope.register.enrollee_no_siblings.$invalid
				&& !$scope.register.enrollee_indigenous.$invalid;
				
				/* test */		
				
				if ($scope.register.enrollee_stat.$valid) {				
					if ($scope.reg['enrollee_stat'] == 2) {						
						$scope.isInvalid += !$scope.register.enrollee_old_school_type.$invalid
						&& $scope.register.enrollee_old_school_name.$invalid;						
					}				
				}
				
				if ($scope.register.enrollee_indigenous.$valid) {
					if ($scope.reg['enrollee_indigenous'] == 'yes') {				
						$scope.isInvalid += !$scope.register.enrollee_ethnicity.$invalid;
					}
				}			
								
				if (!$scope.isInvalid) {
					$.growlUI('Notification', 'Required fields need to be filled up.');	
					return;
				}						
				
				$scope.reg['registered_online'] = 'yes';
				if (isNaN($scope.reg['enrollee_father_income'])) $scope.reg['enrollee_father_income'] = 0;
				if (isNaN($scope.reg['enrollee_mother_income'])) $scope.reg['enrollee_mother_income'] = 0;
				if (isNaN($scope.reg['enrollee_guardian_income'])) $scope.reg['enrollee_guardian_income'] = 0;				
				
				bUI('Please wait...');				
				$http({
				  method: 'POST',
				  url: 'ajax.php?p=register',
				  data: $scope.reg,
				  ...
				  $window.location.href = 'registered.php?id='+response.data;
			...
			$scope.copyFatherInfo = function() {
			
				$scope.chkFatherInfo = $scope.register.enrollee_father_lastname.$valid
				&& $scope.register.enrollee_father_firstname.$valid
				&& $scope.register.enrollee_father_job.$valid
				&& $scope.register.enrollee_father_income.$valid;
				
				if (!$scope.chkFatherInfo) return;
				
				$scope.reg.enrollee_guardian_lastname = $scope.reg.enrollee_father_lastname;
				$scope.reg.enrollee_guardian_firstname = $scope.reg.enrollee_father_firstname;
				$scope.reg.enrollee_guardian_middlename = $scope.reg.enrollee_father_middlename;
				$scope.reg.enrollee_guardian_extname = $scope.reg.enrollee_father_extname;
				$scope.reg.enrollee_guardian_job = $scope.reg.enrollee_father_job;
				$scope.reg.enrollee_guardian_income = $scope.reg.enrollee_father_income;
				
			}
			
			$scope.copyMotherInfo = function() {
			
				$scope.chkMotherInfo = $scope.register.enrollee_mother_lastname.$valid
				&& $scope.register.enrollee_mother_firstname.$valid
				&& $scope.register.enrollee_mother_job.$valid
				&& $scope.register.enrollee_mother_income.$valid;
				
				if (!$scope.chkMotherInfo) return;
				
				$scope.reg.enrollee_guardian_lastname = $scope.reg.enrollee_mother_lastname;
				$scope.reg.enrollee_guardian_firstname = $scope.reg.enrollee_mother_firstname;
				$scope.reg.enrollee_guardian_middlename = $scope.reg.enrollee_mother_middlename;
				$scope.reg.enrollee_guardian_extname = $scope.reg.enrollee_mother_extname;
				$scope.reg.enrollee_guardian_job = $scope.reg.enrollee_mother_job;
				$scope.reg.enrollee_guardian_income = $scope.reg.enrollee_mother_income;
		...
		function unavailable() {
			$.blockUI({
				message: '<h1 style="line-height: 50px;">The Online Registration is unavailable at the moment.  Please visit our administration office for inquiries.</h1><br><button type="button" class="btn btn-lg btn-primary" onClick="home();">Go Back</button>',
				css: {
				width: '80%',
				left: '10%',
				border: 'none', 
		...
		function home() {
			location.href = "http://lzds.edu.ph";
		}
